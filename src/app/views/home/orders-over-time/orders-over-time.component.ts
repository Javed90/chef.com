...
@Component({
  ...
  styleUrls: ['./orders-over-time.component.css']
})
export class OrdersOverTimeComponent implements OnInit {
 ...
 filePath:string = environment.imagePath;
  ...
  getOrderOvertimeData(){
    this.spinner.show();
    this.dashboardService.currentObject.subscribe(res=> this.orderOverObj = res);
    if(this.orderOverObj){
      this.OrdersOverTimeData(this.dateData);
    }else{
      const data:any = this.dashboardService.getOrdersOTData();
      this.orderOverTime = JSON.parse(data);
  ...
  productTypeSearch(event:any,type:any){
    ...
    this.customOverSearch = this.searchString.transform(type,this.orderOverTime.graphResponse,stringVal);
    this.p = 1;
    this.total = this.customOverSearch?.length;
  }
...
exportAll(){
  ...
  this.dashboardService.orderOverTime(this.dateData).subscribe(res=>{
   if(res.success){
